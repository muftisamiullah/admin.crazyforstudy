const WebBookRoutes = require('./WebBookRoutes.js');
const WebChapterRoutes = require('./WebChapterRoutes.js');
const CategoryRoutes = require('./CategoryRoutes.js');
const StudentAuthRoutes = require('./StudentAuthRoutes.js');
const TutorAuthRoutes = require('./TutorAuthRoutes.js');
const WebReviewRoutes = require('./WebReviewRoutes.js');
const WebFaqRoutes = require('./WebFaqRoutes.js');
const WebSubjectRoutes = require('./WebSubjectRoutes.js');
const WebPaymentRoutes = require('./WebPaymentRoutes.js');
const StudentRoutes = require('./StudentRoutes.js');
const WebAssignmentRoutes = require('./WebAssignmentRoutes.js');
const WebQuestionRoutes = require('./WebQuestionRoutes.js');


module.exports = {
    WebBookRoutes,
    WebChapterRoutes,
    CategoryRoutes,
    StudentAuthRoutes,
    TutorAuthRoutes,
    WebReviewRoutes,
    WebFaqRoutes,
    WebSubjectRoutes,
    StudentRoutes,
    WebPaymentRoutes,
    WebAssignmentRoutes,
    WebQuestionRoutes,
}
...
export default function UploadChapters() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      if (data && data.problems === undefined) {
        ...
        response = await axios.post(
          `${API_URL}chapter/update-chapter-csv`,
          formDataUpload,
          options
        );
      }
      ...
      if (response && response.data.success) {
        ...
        setLoading(false);
        history.push(
          `/books-chapters/${params.isbn}/${params.book_name}/${params.book_id}`
        );
      } else {
        errorDispatch({ type: "SET_ERROR", payload: response.data.errors });
        setBtnDisbaled(false);
        setLoading(false);
      }
  ...
  useEffect(() => {
    ...
    let timerSuccess = setTimeout(
      () => errorDispatch({ type: "SET_SUCCESS", payload: "" }),
      1500
    );
    return () => {
  ...
  const handleImportQuestions = async () => {
    ...
    const response = await axios.get(
      `https://backup.crazyforstudy.com/api/get-book-question.php?accessKey=crazyforstudy&isbn=${isbn}`
    );
    if (response) {
  ...
  const handleUploadQuestion = async () => {
    ...
    const response = await axios.post(
      `${API_URL}chapter/import-data`,
      questions,
      options
    );
    if (response.status === 201) {
      setSubmitData(false);
      addToast("Books Data Uploaded successfully", {
        appearance: "success",
        autoDismiss: true,
      });
      history.push(`/books/upload-chapters`);
                ...
                <div className="row">
                  ...
                  <div className="pl-0">
                    ...
                    >
                      {!apiData
                        ? "Upload Data Via CSV"
                        : "Import Book Questions Via API"}
                    </button>
                  </div>
                  {!apiData && (
                    <div className="pl-0 ml-2">
                      <a
                        href="/sampledata/chapter_upload-9780131453401.csv"
                        className="btn btn-sm dark"
                        download
                      >
                  ...
                  {!apiData && (
                    <>
                      ...
                      >
                        ...
                        {data && data.book_isbn && (
                          <Link
                            className="btn btn-sm dark pull-right"
                            to={`/books-chapters/${params.isbn}/${params.book_name}/${params.book_id}`}
                          >
                            View Question
                          </Link>
                        )}
                      ...
                      <div className="col-md-12 row no-gutter mt-2">
                          ...
                          <div className="col-md-3 pl-0">
                            ...
                            <Form method="POST" encType="multipart/form-data">
                              <Form.Group className="col-md-12 pl-0">
                                <Form.Label>Choose .csv format only</Form.Label>
                                <Form.Control
                                  name="books"
                                  autoComplete="off"
                                  type="file"
                        ...
                        <div className="col-md-9 pl-0">
                          <p>
                            <b>
                              All Chapters Questions {data?.problems?.length}{" "}
                            </b>
                          </p>
                          <hr className="mb-1" />
                          {data?.error === true && (
                    ...
                    <div className="col-md-12 row no-gutter p-0 mt-2">
                      <div className="col-md-12">
                        ...
                        {questions?.length > 0 && (
                          <div className="pull-right">
                            {questions?.length > 1 ? (
                              <button
                                className="btnb tn-sm ml-2 dark"
                            ...
                            <button
                              className="btn dark btn-sm ml-2"
                              onClick={(e) => {
                                e.preventDefault();
                                history.push(
                                  `/books/${singleBook?.subject_name}/${singleBook?.sub_subject_name}/${singleBook?.sub_subject_id}`
                                );
                              }}
                            >
                              <span className="fa fa-times mr-2"></span>Cancel
                            </button>
                          </div>
                        )}
                      </div>
                      ...
                      <div className="col-md-12 table-responsive">
                        <div className="table table-bordered">
                          <div
                            className="col-md-12 mt-2 pb-2 pt-2"
                            style={{
                              display: "flex",
                          ...
                          >
                            {questions?.length > 1 &&
                              questions?.map((ques) => {
                                return (
                                  <div
                                    className="col-md-12 mb-2 mt-2 pl-0 pb-2"
                                    key={ques?.uuid}
                                    id={ques?.uuid}
                                    style={{
                                      display: "flex",

...
import useSubjectReviewsQA from '../hooks/useSubjectReviewsQA';
import {useMutation, useQueryClient} from 'react-query'
import axios from 'axios'
import * as cons from '../../../Helper/Cons.jsx'
import Rating from 'react-rating';
import ReviewSubjectHeadingQA from './ReviewSubjectHeadingQA'
import { useToasts } from 'react-toast-notifications';
import Breadcrumb from './SeoBreadCrumbSubjectQA';
import DatePicker from "react-datepicker";
import {s3Path} from '../../../Helper/ApiUrl'

export default function UpdateSubjectStudentReviewsQA() {

const history = useHistory();
const params = useParams();
const { addToast } = useToasts();
const {state} = useContext(AuthContext);
const {data, isLoading} = useSubjectReviewsQA();
...
const mutation = useMutation(formDataUpload => {
        return axios.patch(`${API_URL}subject/save-reviews-qa/${params.id}`, formDataUpload, options)
    },{
    onSuccess: () => {
        setLoading(false);
...
<div className="dash-main-head">
    <h2>Manage QA Subject Reviews</h2>
</div>
                        ...
                        return (
                            ...
                            key={review._id}>
                                ...
                                <hr className="mt-1 mb-2"/>
                                    <ReviewSubjectHeadingQA review={review} />
                            </div>
                        )
                    })}
                </div>

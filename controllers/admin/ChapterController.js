...
// const fileUpload = require("./FileUpload1.js");
...
const getBookExercises = async (req, res) => {
  try {
    ...
    problems = await getBooksProblems(
      req.params.isbn,
      req.params.chapter_no,
      req?.params?.section_no
    );
    res.status(200).json({
      isbn: req.params.isbn,
      chapter_no: req.body.chapter_no,
...
const RemoveBookChapters = async (req, res) => {
  try {
    await Chapter.deleteMany({ book_isbn: req.params.isbn }).then(
      (response) => {
        return res.status(201).json({
...
const getQCData = async (req, res) => {
  try {
    ...
    const approved_qc = await Chapter.count({
      book_isbn: req.params.isbn,
      flag: "approved",
    });
    const rejected_qc = await Chapter.count({
      book_isbn: req.params.isbn,
      flag: "rejected",
    });
    const reworked_qc = await Chapter.count({
      book_isbn: req.params.isbn,
      flag: "reworked",
    });
    const pending_qc = await Chapter.count({
      book_isbn: req.params.isbn,
      flag: "answered",
    });
    const total_qc = await Chapter.count({
      book_isbn: req.params.isbn,
      flag: "notassigned",
    });
    res.status(201).json({
...
const BartelbyImportChapter = async (req, res) => {
  try {
    ...
    if (
      updatedSection?.total_section_uploaded === updatedSection?.total_sections
    ) {
      await BartelbyChapter.findOneAndUpdate(filterupdate, {
        uploaded: 1,
      });
...
const BartelbyClearChapters = async (req, res) => {
  try {
    let section_id = req?.body?.section_id;
    let book_isbn = req?.body?.book_isbn;
    const total_chapters = await Chapter.count({
      book_isbn: book_isbn,
      section_id: section_id,
      source: "bartelby",
    });
    ...
    await Book.findOneAndUpdate(
      { ISBN13: book_isbn },
      { total_question: +uploaded?.total_question - +total_chapters }
    );
    res.status(201).json({
      error: false,
      status: 201,
...
const BartelbyClearAllChapters = async (req, res) => {
  try {
    let book_isbn = req?.body?.book_isbn;
    await BartelbyChapter.updateMany(
      { book_isbn: book_isbn },
      { uploaded: 0, total_question: 0, total_uploaded: 0 },
      { multi: true }
    );
...
const BartelbyUpdateChaptersAnswer = async (req, res) => {
  try {
    ...
    await Chapter.findByIdAndUpdate(
      { _id: question_id, source: "bartelby" },
      updateData
    );
    if (books?.bartlyby_imported) {
      const BChapterSectionID = ChapterData?.section_id;
      const BChapter = await BartelbyChapter.findOne({
        section_id: BChapterSectionID,
      });
      const ChapterCount = await BartelbyChapter.find({
        section_id: BChapterSectionID,
      }).countDocuments();
      ...
      if (quizChapter?.total_section > 0) {
        ...
        await QuizletChapter.findById(chapter_id, function (err, result) {
          if (err) {
            ...
            result.sections.id(section_id).answer_uploaded =
              total_quizletSection;
...
const SaveQuizletChapters = async (req, res) => {
  try {
    ...
    if (count === 0) {
      ...
      await Book.findOneAndUpdate(
        { ISBN13: req?.body?.book_isbn },
        { quizlet_imported: 1, bartleby_imported: 0 }
      );
      res.status(201).json({
        error: false,
        status: 201,
...
const UpdateAnswerTbs = async (req, res) => {
  try {
    ...
    if (response) {
      ...
      const d = await Notify.updateMany(
        { data_Id: req.params.q_id, type: "TBS", user_Id: { $in: ids } },
        {
          title: data.question,
      ...
      const output = emails.askTbsSolutionSolved(
        response.book_name,
